...
# They are named in capitals bc they are constants!
...
class Enemy(Ship):
    ...
    def __init__(self, x, y, color, health=100):
        super().__init__(x, y, health)
        self.ship_img, self.laser_img = self.COLORRM[color]
        self.mask = pygame.mask.from_surface(self.ship_img)
...
def main():
    ...
    def redraw_window():
        WIN.blit(BG, (0,0)) # blit is a method that takes one of d py images is and draws it to d window @ location we defined. 
        # draw text
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wavelength):
                enemy = Enemy(random.randrange(50, WIDTH-100), random.randrange(-1500, -100), random.choice(["green", "red", "blue"]))
        ...
        if keys[pygame.K_w] and player.y - playervel > 0: #up
            player.y -= playervel
        if keys[pygame.K_s] and player.y + playervel + player.get_height() + 15 < HEIGHT: #down
            player.y += playervel
        ...
        for enemy in enemies:
            enemy.move(enemyvel)
            enemy.lasermover(laservel, player)
        ...
        player.lasermover(-laservel, enemies)
